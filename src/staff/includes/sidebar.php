...
if (isset($pageConfig)) {
    if (isset($pageConfig['sidebar'])) {
        if (isset($pageConfig['sidebar']['active'])) {
            $active = $pageConfig['sidebar']['active'];
        }
        if (isset($pageConfig['sidebar']['links'])) {
            $links = $pageConfig['sidebar']['links'];
        }
        if (isset($pageConfig['sidebar']['title'])) {
...
<header class="sidebar">
    <svg width="70" viewBox="0 0 94 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
    ...
    <div class="links">
        <?php foreach ($links as $index => $link) : ?>
            <a href="<?php echo $link['href'] ?>" <?php echo $index === $active - 1 ? 'class="active"' : '' ?>><?php echo $link['text'] ?></a>
        <?php endforeach; ?>
...
<script>
    const btn = document.querySelector(".sidebar_toggle")
